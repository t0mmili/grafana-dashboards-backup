...
counter=$(terraform output -json -state="$tf_state" | jq -j '.grafana_dashboards.value.uid | length')

if [ $counter -gt 0 ]; then
	i=0
	while [ $i -lt $counter ]; do
    uid=$(terraform output -json -no-color -state="$tf_state" | jq -j --arg i "$i" '.grafana_dashboards.value.uid[$i | tonumber]')
    config=$(terraform output -json -no-color -state="$tf_state" | jq -j --arg i "$i" '.grafana_dashboards.value.config[$i | tonumber]')
    echo $config > "$out_dir$([ ${out_dir#${out_dir%?}} != '/' ] && printf '/')$uid.json"
		let "i+=1"
	done 
else
